...
alert(user.fullName); // John Smith, 일반 프로퍼티처럼 사용 가능
```
...
이 숨김 프로퍼티 값은 null이거나 다른 객체에 대한 참조가 되는데, 다른 객체를 참조하는 경우 참조 대상을 '프로토타입'이러 부름

![image](https://github.com/user-attachments/assets/3399bd78-862f-4aca-9ea6-1f3b42a1e24f)

프로토타입 상속 : object에서 프로퍼티를 읽으려는데 해당 프로퍼니가 없으면 자바스크립트는 자동으로 프로토타입에서 프로퍼티를 찾음
